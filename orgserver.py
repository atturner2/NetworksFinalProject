...
class ClientThread(Thread):
    ...
    def openAndReadDatFile(self, data):
        file = open('datfiles/org.dat')
        print "IN the open file function"
        message = "<OxFF, org, 'Host Not Found'>"
        for line in file:
             temp = line.split()
             domain = temp[0]
             ...
             if lowereddata.strip() == domain.strip():
                 ...
                 message = "<0x00, org, " + temp[1] + ">"
        return message
    #Here we have the org server. All it does is recieve requests and then check the org
...
while True:
    tcpServer.listen(4)
    print "Multithreaded Python org server : Waiting for connections from TCP clients..."
